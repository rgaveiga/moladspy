...
class Adsorption(Hybrid):
    ...
    def _update(self):
        ...
        for molecule in self.adsorbed_molecules:
            ...
            if abs(dz)>0.0:
                molecule.displace(array([0.0,0.0,dz]),update=True,update_owner=False)
    ...
    def origin(self,val):
        ...
        if isinstance(val,ndarray) and val.shape[0]==3:
            ...
            self._components["substrate"].displace(disp,update=True,update_owner=False)
        
            for molecule in self.adsorbed_molecules:
                molecule.displace(disp,update=True,update_owner=False)
